...
若一个模块可以当作函数调用，则使用[`module-function.d.ts`](module-function.d.ts.md)。
...
请务必阅读[脚注："ES6 对模块调用签名的影响"]()。

如果一个模块可以使用`new`来构造，则使用[`module-class.d.ts`](module-class.d.ts.md)。
...
请务必阅读[脚注："ES6 对模块调用签名的影响"]()，它同样适用于这类模块。

如果一个模块既不可以调用，又不可以构造，那么就使用[`module.d.ts`](module.d.ts.md)。
...
使用[`module-plugin.d.ts`](module-plugin.d.ts.md)模版。
...
使用[`global-plugin.d.ts`](global-plugin.d.ts.md)模版。
...
使用[`global-modifying-module.d.ts`]global-modifying-module.d.ts.md)模版。
...
顶层的模块对象永远不能够被调用。
最常见地解决方案是为可调用的/可构造的对象定义一个`default`导出；
